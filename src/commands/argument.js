...
class Argument {
	...
	async obtain(msg, value, promptLimit = Infinity) {
		...
		while(!valid || typeof valid === 'string') {
			...
			prompts.push(await msg.say({embed: {
				description: stripIndents`${empty ? this.prompt : valid ? valid : `You provided an invalid ${this.label}. Please try again.`}`,
				color: 0xFFBF00,
				author: {
					name: msg.author.tag,
				...
				fields: [
					{
						name: `\u200b`,
						value: `Respond with **\`cancel\`** to cancel this command.`
					}
				],
				footer: {
			...
			const responses = await msg.channel.awaitMessages(msg2 => msg2.author.id === msg.author.id, {maxMatches: 1, time: wait});
