...
class Command {
	...
	hasPermission(message, ownerOverride = true) {
		...
		if(this.ownerOnly && (ownerOverride || !this.client.isOwner(message.author))) return `Command (\`${this.name}\`) can only be used by the bot developer(s)`;

		if(message.channel.type === 'text' && this.userPermissions) {
			const missing = message.channel.permissionsFor(message.author).missing(this.userPermissions);
			if(missing.length > 0) {
				if(missing.length === 1) return `Command (\`${this.name}\`) requires you to have the "${permissions[missing[0]]}" permission.`; 
				return oneLine`
					Command (\`${this.name}\`) requires you to have the following permissions:
	...
	static validateInfo(client, info) { // eslint-disable-line complexity
		...
		if(info.dmOnly && info.guildOnly){
			throw new TypeError(`Command ${info.name} can't have "dmOnly" and "guildOnly" enabled.`);
		}
