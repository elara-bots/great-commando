...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(!hasPermission || typeof hasPermission === 'string') {
			...
			if(typeof hasPermission === 'string') return send(hasPermission, 0xFF0000);
			else return send(`Command (\`${this.command.name}\`) you don't have permission to use.`, 0xFF0000);
		}
		...
		if(throttle && throttle.usages + 1 > this.command.throttling.usages) {
			...
			return send(`You may not use the \`${this.command.name}\` command again for another ${remaining.toFixed(1)} seconds.`, 0xFF0000);
		}
		...
		if(!args && this.command.argsCollector) {
			...
			if(result.cancelled) {
				if(result.prompts.length === 0) {
					...
					return send(err.message, 0xFF0000);
				}
				return send('Cancelled command.', 0xFF0000);
			}
			args = result.values;
		}
