				...
				embed.fields = [
					{
						...
						value: valid.map(c => `\`${c}\``).join(", ")
					}
				]
			};
			if(msg.channel.type === "dm") return msg.channel.send({embed: embed}).catch(() => {});
			if(msg.channel.permissionsFor(msg.client.user.id).has("EMBED_LINKS")) return msg.channel.send({embed: embed}).catch(() => {});
			return msg.channel.send(`**__INFO__**\n\`\`\`js\n${error}\`\`\``).catch(() => {});
        }
